...
void ViewPort_dump(struct ViewPort *v, const char *name) {
  ...
  SDL_RenderReadPixels(v->ren, NULL, surface->format->format, surface->pixels, surface->pitch);
...
struct ViewPort *ViewPort_query(int w, int h, callback_t c, void *data, bool resize) {
  if (vg)
    return vg;
  vg = (struct ViewPort *)calloc(1, sizeof(struct ViewPort));
  ...
  atexit(SDL_Quit);
  vg->screen = SDL_CreateWindow("SDL window", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, w, h,
  ...
  vg->texture = SDL_CreateTexture(vg->ren, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, w, h);
